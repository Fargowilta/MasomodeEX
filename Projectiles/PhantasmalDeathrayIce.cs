...
namespace MasomodeEX.Projectiles
{
    public class PhantasmalDeathrayIce : ModProjectile
    {
		...
		{
			DisplayName.SetDefault("Glacial Deathray");
		}
        ...
        public override void AI()
        {
            ...
            //for (int i = 0; i < array3.Length; i++) array3[i] = 3000f;
            float num807 = 0f;
            int num3;
        ...
        {
            target.AddBuff(BuffID.Chilled, Main.rand.Next(3600));
            target.AddBuff(BuffID.Frostburn, Main.rand.Next(300, 600));
            target.AddBuff(BuffID.Frozen, Main.rand.Next(180));
        }
    }
}